...
public class UserControllerTests {
	...
	public void should_get_all_users() throws Exception {
		...
		this.mockMvc.perform(get("/api/users")).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)));
	}
	...
	public void should_get_user_by_id() throws Exception {
		...
		this.mockMvc.perform(get("/api/users/" + existingUser.getId())).andExpect(status().isOk())
	...
	public void should_update_user() throws Exception {
		...
		this.mockMvc
				.perform(put("/api/users/" + existingUser.getId()).contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(existingUser)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.id", is(existingUser.getId())))
				.andExpect(jsonPath("$.name", is(existingUser.getName())))
				.andExpect(jsonPath("$.email", is(existingUser.getEmail())));
	}
